...
def scan_low_with_huge_vol(opening, closing, high, low, vol):
	...
	for index in xrange(trace_index, len(opening)):
		...
		if min(low[index - trace_index: index]) >= low[index] and min(closing[index - trace_index: index]) >= closing[index] and vol[index] > vol_ratio * sum(vol[index - trace_index: index - 1]) / trace_index:
			lhw_arr.append(1)
