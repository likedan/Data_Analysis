...
found_trade_symbol = False
if current_element[:3] == trading_symbol[:3] and current_element[-3:] == trading_symbol[-3:]:
	pass
else:
	for symbol in tradable_elements.keys():
		if symbol[:3] == trading_symbol[:3] and symbol[-3:] == trading_symbol[-3:]:
			if trading.trade_element(symbol):
				found_trade_symbol = True
				break
...
if found_trade_symbol:
	...
	while True:
		...
		if current_minute != last_minute:
			...
			features_arr = []
			for x in range(2,31):
				features_arr.append(get_complex_features(close[-x], -x))
