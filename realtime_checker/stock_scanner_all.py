...
def scan_stocks(date1, date2, symbols, thread_num):
	while True:
		try:
			for s in symbols:
				try:
...
p = threading.Thread(target=scan_stocks, args=(date1, date2, symbols, "for",))
p.start()
p1 = threading.Thread(target=scan_stocks, args=(date1, date2, reversed(symbols),"back",))
p1.start()